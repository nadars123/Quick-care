...
function SpecialitiesTable() {
  ...
  const getSpecialities = async () => {
    try {
      const response = await axios.get(
        "http://localhost:7000/api/v1/speciality"
      );
  ...
  const handleSave = async () => {
    try {
      if (currentSpeciality?._id) {
        await axios.put(
          `http://localhost:7000/api/v1/speciality/${currentSpeciality._id}`,
          currentSpeciality
        );
        toast.success('Speciality Updated Successfuly')
      } else {
        await axios.post(
          "http://localhost:7000/api/v1/speciality",
          currentSpeciality
              ...
              toast.success("Speciality Created Successfuly");+
      getSpecialities();
      setModalOpen(false);
    } catch (error) {
  ...
  const handleDelete = async (_id: string) => {
    try {
        await axios.delete(`http://localhost:7000/api/v1/speciality/${_id}`);
