...
function Main() {
  ...
  const getPatients = async () => {
    try {
      const response = await axios.get(
        "http://localhost:7000/api/v1/user/getAllUsersBySpecificRole/patient"
      );
  ...
  const initTabulator = () => {
    if (tableRef.current && patients) {
      tabulator.current = new Tabulator(tableRef.current, {
        data: patients, // Use the doctors data from the API
        layout: "fitColumns",
        ...
        columns: [
...

          {
            title: "Age",
            minWidth: 100,
            field: "id_patient.birth_date",
            vertAlign: "middle",
            formatter: (cell) => {
              const birthDate = new Date(cell.getValue());
        ...
        <div className="grid grid-cols-12 gap-y-10 gap-x-6">
          <div className="col-span-12">
            ...
            {patients.length > 0 ? (
                    ...
                    >
                      ...
                      <FormInline className="flex-col items-start xl:flex-row xl:items-center gap-y-2">
                        ...
                        >
                          ...
                          <option value="id_patient.phone_number">
                            Number
                          </option>
                          <option value="id_patient.Adresse">Adresse</option>
                        </FormSelect>
                      </FormInline>
                      <FormInline className="flex-col items-start xl:flex-row xl:items-center gap-y-2">
                        <FormLabel className="mr-3 whitespace-nowrap">
              ...
              <div className="w-[70vw] flex gap-3 flex-col justify-center items-center p-7 rounded-sm bg-white">
                <img width={80} src={Nodata} alt="no-data" />
                <span>No Patients founded !</span>
              </div>
            )}
          </div>
        </div>
